// ================================================
// 🚀 REALISTIC BACKTEST ENGINE (FINAL)
// NO DUPLICATE + REAL EXECUTION
// ================================================

export function runBacktest(candles, strategyFn) {
  const trades = [];

  let activeTrade = null;
  let cooldown = 0;

  for (let i = 100; i < candles.length - 1; i++) {
    const history = candles.slice(0, i);
    const current = candles[i];
    const next = candles[i + 1];

    // ================================
    // EXIT LOGIC (REALISTIC)
    // ================================
    if (activeTrade) {
      const { dir, sl, tp } = activeTrade;

      const high = current.high;
      const low = current.low;

      let exitType = null;

      // SL/TP race (intrabar approximation)
      if (dir === "CALL") {
        if (low <= sl && high >= tp) {
          exitType = Math.abs(sl - current.open) < Math.abs(tp - current.open) ? "SL" : "TP";
        } else if (low <= sl) {
          exitType = "SL";
        } else if (high >= tp) {
          exitType = "TP";
        }
      }

      if (dir === "PUT") {
        if (high >= sl && low <= tp) {
          exitType = Math.abs(sl - current.open) < Math.abs(tp - current.open) ? "SL" : "TP";
        } else if (high >= sl) {
          exitType = "SL";
        } else if (low <= tp) {
          exitType = "TP";
        }
      }

      if (exitType) {
        trades.push({
          ...activeTrade,
          exitType,
          exitPrice: exitType === "TP" ? tp : sl,
          exitTime: current.time,
        });

        activeTrade = null;
        cooldown = 3; // prevent overtrading
      }

      continue;
    }

    // ================================
    // COOLDOWN
    // ================================
    if (cooldown > 0) {
      cooldown--;
      continue;
    }

    // ================================
    // ENTRY (NEXT CANDLE ONLY)
    // ================================
    const signal = strategyFn(history);
    if (!signal) continue;

    activeTrade = {
      ...signal,
      entryPrice: next.open, // realistic entry
      entryTime: next.time,
    };
  }

  const total = trades.length;
  const wins = trades.filter(t => t.exitType === "TP").length;

  return {
    total,
    wins,
    losses: total - wins,
    winrate: total ? ((wins / total) * 100).toFixed(2) : "0.00",
    trades,
  };
}
