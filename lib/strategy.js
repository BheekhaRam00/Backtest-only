// ================================================
// 🚀 SMART REGIME STRATEGY (V3.5 FINAL)
// TREND + RANGE + VOL + FAKE BREAKOUT + AI SCORE
// ================================================
...
function detectMarket(c) {
  ...
  if (!atrNow) return "NONE";

  const volRatio = atrPrev ? atrNow / atrPrev : 1;

  if (adxVal >= 28 && volRatio > 0.9) return "STRONG_TREND";
  if (adxVal >= 24) return "WEAK_TREND";

  if (adxVal <= 18 && volRatio < 1.2) return "RANGE";
  if (volRatio > 1.5) return "HIGH_VOL";
...
function fakeBreakout(c) {
  ...
  const emaF = ema(c, EMA_FAST).at(-1);
  const emaS = ema(c, EMA_SLOW).at(-1);

  // ❌ disable in strong trend
  if (Math.abs(emaF - emaS) / emaS > 0.002) return null;
  ...
  if (cur.high > prev.high && cur.close < prev.high) {
    return { dir: "PUT", entry: cur.close, sl: cur.high, tp: cur.close - range * 1.5 };
  }

  if (cur.low < prev.low && cur.close > prev.low) {
    return { dir: "CALL", entry: cur.close, sl: cur.low, tp: cur.close + range * 1.5 };
  }
...
function scoreTrade(c, trade) {
  ...
  if (trade.dir === "CALL" && emaF > emaS) score += 0.1;
  if (trade.dir === "PUT" && emaF < emaS) score += 0.1;

  return score;
}

// ================================
// CORE GENERATOR (FINAL)
// ================================
function generate(c) {
  ...
  if (market === "STRONG_TREND") {
    ...
    trade = fakeBreakout(c) || trendLogic(c);
  }
  else if (market === "RANGE") {
    trade = rangeLogic(c);
  }
  else if (market === "HIGH_VOL") {
  ...
  let minScore = 0.55;
  if (market === "STRONG_TREND") minScore = 0.5;
  if (market === "RANGE") minScore = 0.6;
  if (market === "HIGH_VOL") minScore = 0.65;
